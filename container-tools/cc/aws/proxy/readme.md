...
1. `bazel build //cc/aws/proxy/src:proxy //cc/aws/proxy/src:proxify //cc/aws/proxy/src:proxy_preload //cc/aws/proxy/src:socket_vendor`
1. Under directory `bazel-bin/cc/aws/proxy/src`, find the built binaries `proxy`,
...
1. AmazonLinux2 (AL2) uses an old version of glibc. Newer compilers with newer
   glibc on your build machine may generate binaries that are not runnable on AL2.
   We use a custom LLVM toolchain and sysroot specified in `MODULE.bazel` to
   ensure compatibility with AL2 and reproducibility, removing the need for a
   containerized build.
